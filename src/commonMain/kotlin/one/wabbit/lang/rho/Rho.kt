...
class Rho {
    ...
    class RuleNode(
        val id: Rule,
        val orderedAntecedents: List<AntecedentPlan>,
    ...
    private fun factNodeOf(fact: Fact): FactNode {
        ...
        return factMap.getOrPut(fact) { FactNode(fact).also(::addFactConstIndex) }
    }
    ...
    private fun removeAntecedentTrigger(antecedent: AntecedentPlan, rule: Rule) {
        fun MutableList<AntecedentTrigger>.removeMatching() {
    ...
    private fun removeFactNode(factNode: FactNode) {
        facts[factNode.id.predicate]?.let { factMap ->
            factMap.remove(factNode.id)
            if (factMap.isEmpty()) {
                facts.remove(factNode.id.predicate)
            }
        }
    ...
    fun add(startRule: Rho.Rule): Set<Fact> {
        ...
        val antecedentPlans = startRule.antecedents.toList().map(::buildAntecedentPlan)
        val ruleNode = RuleNode(startRule, antecedentPlans.sortedByDescending { it.specificity })
        rules[startRule] = ruleNode

        for (a in antecedentPlans) {
            addAntecedentTrigger(a, startRule)
        }
    ...
    fun remove(rule: Rho.Rule): Set<Fact> {
        ...
        for (a in rule.antecedents) {
            removeAntecedentTrigger(buildAntecedentPlan(a), rule)
        }
    ...
    private fun processQueue(queue: ArrayDeque<Pair<Rho.Rule, SubstMap>>): Set<Fact> {
        ...
        while (queue.isNotEmpty()) {
            ...
            fun installDerivedFact(fact: Fact, usedFacts: ConsList<FactNode>) {
                ...
                if (!alreadyExists) {
                    val matchedRules = LinkedHashSet<Rho.Rule>()

                    fun tryEnqueueTrigger(trigger: AntecedentTrigger) {
                        if (!trigger.antecedent.term.canUnify(fact)) return
                        if (!matchedRules.add(trigger.rule)) return
                        if (!inQueue.contains(trigger.rule)) queue.add(trigger.rule to arrMapOf())
                        if (DEBUG) println("    Marking `${trigger.rule}` dirty")
                    }

                    for (trigger in antecedentAnyToRule[fact.predicate] ?: emptyList()) {
